...
//  2023-02-23: Accept SDL_GetPerformanceCounter() not returning a monotonically
//  increasing value. (#6189, #6114, #3644) 2023-02-07: Implement IME handler
...
//  'nullptr' instead of 'NULL' as per our switch to C++11. 2022-09-26: Inputs:
//  Disable SDL 2.0.22 new "auto capture" (SDL_HINT_MOUSE_AUTO_CAPTURE) which
//  prevents drag and drop across windows for multi-viewport support + don't
//  capture when drag and dropping. (#5710) 2022-09-26: Inputs: Renamed
...
bool ImGui_ImplSDL2_ProcessEvent(const SDL_Event *event) {
  ...
  case SDL_KEYUP: {
    ...
    io.SetKeyEventNativeData(
      ...
      event->key.keysym.scancode
    ); // To support legacy indexing (<1.87 user code). Legacy backend uses
...
static void ImGui_ImplSDL2_UpdateMouseData() {
  ...
  if (is_app_focused) {
    ...
    if (bd->MouseCanUseGlobalState && bd->MouseButtonsDown == 0) {
      ...
      io.AddMousePosEvent(
        (float)(mouse_x_global - window_x), (float)(mouse_y_global - window_y)
      );
    }
  }
}
...
#define MAP_ANALOG(KEY_NO, AXIS_NO, V0, V1)                                    \
  ...
  MAP_ANALOG(
    ImGuiKey_GamepadLStickUp, SDL_CONTROLLER_AXIS_LEFTY, -thumb_dead_zone, -32768
  );
  MAP_ANALOG(
